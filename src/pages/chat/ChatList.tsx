      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        {filteredRooms.length > 0 ? (
          <div className="divide-y divide-slate-100 dark:divide-slate-700/50">
            {filteredRooms.map((room) => (
              <button
                ...
                className="w-full p-5 md:p-6 flex items-center gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all text-left group"
              >
                <div className="relative shrink-0">
                  <div className="w-12 h-12 md:w-14 md:h-14 bg-blue-100 dark:bg-blue-900/50 rounded-2xl flex items-center justify-center text-blue-600 dark:text-blue-400 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1">
                    <h4 className="font-black text-slate-900 dark:text-white truncate pr-4 uppercase italic text-sm">
                      {room.contact_name}
                    </h4>
                    <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 flex items-center gap-1 whitespace-nowrap shrink-0">
                      <Clock size={12} /> {room.last_message_time ? new Date(room.last_message_time).toLocaleDateString() : ''}
                    </span>
                  </div>
                  
                  <p className="text-blue-600 dark:text-blue-400 text-[10px] font-black uppercase tracking-widest mb-1">
                ...
                <ChevronRight className="text-slate-300 dark:text-slate-600 group-hover:text-blue-600 transition-colors shrink-0" size={20} />
              </button>
            ))}
          </div>
        ) : (
          <div className="p-20 text-center">
            ...
            <h3 className="font-bold text-slate-800 dark:text-slate-200 uppercase italic">Nenhuma conversa encontrada</h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-2">Interaja com os fretes para iniciar negociações.</p>
          </div>
        )}
      </div>
    </div>
