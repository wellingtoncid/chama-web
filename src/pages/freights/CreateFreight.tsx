      ...
      useEffect(() => {
        if (editData) {
            setFormData({
                ...
                dest_city: editData.dest_city || '',
                product: editData.product || '',
                weight: editData.weight !== undefined && editData.weight !== null ? String(editData.weight) : '',
    ...
    useEffect(() => {
        if (formData.origin_state) {
            getCitiesByState(formData.origin_state).then(cities => {
                ...
                // SÓ limpa se houver cidades na lista E a cidade selecionada não estiver nelas
                // E garantimos que não estamos na carga inicial de edição
                if (cities.length > 0 && formData.origin_city && !cities.includes(formData.origin_city)) {
                    if (!editData || formData.origin_city !== editData.origin_city) {
    ...
    useEffect(() => {
        if (formData.dest_state) {
            getCitiesByState(formData.dest_state).then(cities => {
                ...
                if (cities.length > 0 && formData.dest_city && !cities.includes(formData.dest_city)) {
                    if (!editData || formData.dest_city !== editData.dest_city) {
