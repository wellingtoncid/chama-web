...
export default function GroupsPortalFinal() {
  ...
  const renderGrid = () => {
    ...
    const filteredGroups = groups.filter(g => 
      (g.display_location === 'site' || g.display_location === 'both') &&
              ...
              return (
                <div key={`ad-${idx}`} className="group relative bg-[#0F172A] rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col min-h-[420px] border border-slate-800">
                  <div className="absolute inset-0">
                    <img src={item.image_url} className="w-full h-full object-cover opacity-40 group-hover:scale-110 transition-transform duration-[10s]" alt="" />
