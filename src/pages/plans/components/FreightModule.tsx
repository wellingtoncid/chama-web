import { ArrowLeft, Check, Truck, Sparkles } from 'lucide-react';
...
export default function FreightModule() {
  ...
  const freePublishRule = pricingRules.find(r => r.module_key === 'freights' && r.feature_key === 'publish');
      ...
      {hasActivePlan ? (
        <div className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-5 flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <h3 className="font-black uppercase italic text-lg text-emerald-800 dark:text-emerald-300">Plano de Fretes Ativo</h3>
              <Check size={16} className="text-emerald-500" />
            </div>
            <p className="text-sm text-emerald-600 dark:text-emerald-400">Seus fretes estão sendo publicados com os recursos do seu plano</p>
          </div>
        </div>
      ) : freeLimit > 0 ? (
        <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-2xl p-5 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-blue-600 dark:text-blue-400">Você pode publicar até {freeLimit} frete{freeLimit > 1 ? 's' : ''} por mês sem custo</p>
          </div>
        </div>
      ) : (
        <div className="bg-slate-100 dark:bg-slate-800 rounded-2xl p-5 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-400">Assine um plano abaixo para publicar fretes</p>
          </div>
        </div>
      )}
      ...
      {subscriptionPlans.length > 0 && (
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="font-black text-lg text-slate-900 dark:text-slate-100">Planos de Frete</h2>
              <p className="text-xs text-slate-500">Escolha o plano ideal para sua operação</p>
            </div>
          </div>
      ...
      {rules.length > 0 && (
        <div className="bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-black text-sm text-slate-800 dark:text-slate-100">Quer destacar seus fretes?</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">Use boost, urgente e outros recursos avulsos</p>
            </div>
          </div>
          <button
