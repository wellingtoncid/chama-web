      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-col xl:flex-row gap-8">
          <div className="flex-1">
            {loading ? (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {[1,2,3,4,5,6].map(i => <div key={i} className="h-72 bg-slate-100 animate-pulse rounded-[2.5rem]" />)}
              </div>
            ) : (
              <div className="flex flex-col gap-12">
                
                {/* GRID DE CARGAS E ANÚNCIOS */}
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {freights.length > 0 ? (
                    <>
                      {freights.map((freight: any, index: number) => (
                        <React.Fragment key={freight.id}>
                          ...
                          {/* 2. ANÚNCIO VERTICAL INJETADO (Aparece como 2º card da grade) */}
                          {index === 0 && (
                            <div className="relative h-full">
                               <div className="absolute -top-3 left-8 z-20 bg-[#1f4ead] text-white text-[7px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-lg">
                          ...
                          {/* 3. CARD "VER MAIS" (Aparece no final da grade se houver mais cargas) */}
                          {index === freights.length - 1 && remainingCount > 0 && (
                            <Link 
                              to={`/fretes?search=${encodeURIComponent(searchTerm)}`}
                              className="relative overflow-hidden bg-slate-900 border-4 border-slate-800 rounded-[2.5rem] p-8 flex flex-col items-center justify-center text-center group hover:bg-slate-800 transition-all shadow-2xl"
                            >
                    ...
                    <div className="col-span-full text-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-slate-100">
                      <p className="text-slate-400 font-black uppercase tracking-widest italic">Nenhuma carga encontrada</p>
                    </div>
                  )}
                </div>

                {/* ANÚNCIO HORIZONTAL (Divisor de Fluxo) */}
                <div className="pt-4">
                   <div className="flex items-center gap-4 mb-6">
                      <div className="h-[1px] flex-1 bg-slate-200"></div>
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Publicidade Recomendada</span>
                      <div className="h-[1px] flex-1 bg-slate-200"></div>
                   </div>
                   <AdCard 
          ...
          <aside className="hidden xl:block w-80 flex-shrink-0">
            <div className="sticky top-28 space-y-6">
              ...
              {/* Banner CTA Interno */}
              <div className="bg-white p-8 rounded-[3rem] text-slate-900 shadow-xl relative overflow-hidden group border border-slate-100">
                  ...
                  <div className="relative z-10">
                    <div className="bg-slate-900 w-fit p-2 rounded-xl mb-4">
                        <BarChart3 size={20} className="text-orange-500" />
                    </div>

                    <h4 className="font-[1000] italic uppercase leading-none mb-4 text-3xl tracking-tighter text-slate-900">
                    ...
                    <button 
                      ...
                      className="w-full bg-slate-900 text-white py-5 rounded-[1.5rem] font-[1000] uppercase text-[11px] tracking-[0.1em] hover:bg-orange-500 transition-all shadow-xl active:scale-95 flex items-center justify-center gap-2 group/btn"
                    >
                      Anunciar Agora
        ...
        <div className="text-center mt-16">
          <Link to={`/fretes?search=${encodeURIComponent(searchTerm)}`} className="inline-block">
            <Button className="h-16 rounded-2xl px-12 bg-slate-900 hover:bg-[#1f4ead] text-white font-[1000] uppercase italic tracking-widest transition-all shadow-2xl group flex flex-col items-center justify-center">
