...
const CompanyFields = ({ formData, setFormData }: any) => {
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      {/* 2. INFOS CADASTRAIS (GRID LIMPO) */}
      <div className="bg-slate-50 p-10 rounded-[3.5rem] border border-slate-100 grid md:grid-cols-2 gap-8">
          <Input label="Nome Fantasia (Sua Marca)" value={formData.name_fantasy} onChange={(v:any) => handleInputChange('name_fantasy', v)} placeholder="Ex: Logística Avançada S/A" />
          <Input label="CNPJ Oficial" value={formData.cnpj} onChange={(v:any) => handleInputChange('cnpj', v)} placeholder="00.000.000/0000-00" />
          <Input label="Site Institucional" value={formData.website_url} onChange={(v:any) => handleInputChange('website_url', v)} placeholder="www.suaempresa.com.br" />
          <Input label="E-mail Comercial (Para Cotações)" value={formData.commercial_email} onChange={(v:any) => handleInputChange('commercial_email', v)} placeholder="comercial@empresa.com" />
      </div>
